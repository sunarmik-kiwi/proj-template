const StatusCodes = Object.freeze({
   ...
   * to later send an asynchronous response indicating the outcome
   ...
   *
   * This response code means returned meta-information
   * set is not exact set as available from the origin server,
   ...
   *  user should choose one of them. There is no standardized
   *  way to choose one of the responses.
   */
   ...
   *  response has not been modified. So, client can continue to use same cached
   *  version of response.
   */
  NOT_MODIFIED: 304,
  /**
   ...
   *  response code, with the exception that the user agent must
   ...
   *
   * This response code is reserved for future use. Initial aim for
   ...
   *  This response code is probably the most famous one due
   ...
   *  and cannot be used. For example, an API may
   * forbid DELETE-ing a resource. The two mandatory methods,
   *  GET and HEAD, must never be disabled and should
   ...
   *  like Chrome, Firefox 27+, or IE9, use HTTP pre-connection mechanisms
   ...
   * has been permenantly deleted from server, with no forwarding address.
   *  Clients are expected to remove their caches and links to the resource.
   *  The HTTP specification intends this status code to be used
   * for "limited-time, promotional services". APIs
   * should not feel compelled to indicate resources that have
   ...
   *  not be performed on the resource because the server is unable
   *  to store the representation needed to successfully complete
   *  the request. This condition is considered to be temporary.
   *  If the request which received this status code was the result
   *  of a user action, the request MUST NOT be repeated until it
   *  is requested by a separate user action.
   */
   ...
   * /rfcdiff?difftype=--hwdiff&url2=draft-ietf-webdav-protocol-06.txt
   *
   ...
   *
   * Defined in the specification of HTTP/2 to indicate that a server
   *  is not able to produce a response for the combination of scheme
   ...
   *  a gateway to get a response needed to handle the request,
   ...
   *  the problem should be sent. This responses should be used for temporary
